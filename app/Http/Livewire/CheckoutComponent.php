...
class CheckoutComponent extends Component
{
    ...
    public function placeOrder(){
        ...
        if(Str::of($this->subtotal)->contains(',')){
            $order->subtotal =  Str::of($this->subtotal)->remove(',');
        }
        else{
            $order->subtotal = $this->subtotal;
        }

        if(Str::of($this->discount)->contains(',')){
            $order->discount =  Str::of($this->discount)->remove(',');
        }
        else{
            $order->discount = $this->discount;
        }

        if(Str::of($this->tax)->contains(',')){
            $order->tax =  Str::of($this->tax)->remove(',');
        }
        else{
            $order->tax = $this->tax;
        }

        if(Str::of($this->total)->contains(',')){
            $order->total =  Str::of($this->total)->remove(',');
        }
        else{
        ...
        if($this->payment_mode == 'cod'){
            ...
            try{
                ...
                if($charge['status'] == 'succeeded'){
                    ...
                    session()->flash('transaction_error' , 'Transaction has error');
                    $this->thankyou = 0;
                }
            }
            catch(Exception $e){
